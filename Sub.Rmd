...
# Clean and Preproces the Data Sets.
...
```{r}
ppc<-preProcess(baseStrip_2[,-ncol(baseStrip_2)],method=c("BoxCox","center","scale"))
ds<-predict(ppc,baseStrip_2[,-ncol(baseStrip_2)])
```
...
Use the preprocess function to transform the 

```{r}
ppc2<-preProcess(baseStrip_2[,-ncol(baseStrip_2)],method=c("BoxCox","center","scale","pca"),pcaComp=numComp)
reduced_Set<-predict(ppc2,baseStrip_2[,-ncol(baseStrip_2)])
reduced_Set[ncol(reduced_Set)+1]<-baseStrip_2[,ncol(baseStrip_2)]
```